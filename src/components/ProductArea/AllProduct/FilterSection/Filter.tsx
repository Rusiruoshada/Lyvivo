  ...
  const items: Items[] = [
    {
      ...
      children: filters.productType.map((productItem, index) => (
  ...
  const availability: Availability[] = [
    {
      ...
      children: (
        <div className='my-0 py-0'>
          {filters.availability.map((available, index) => (
            <div className='flex gap-2' key={available + index}>
              <input title={available} type='checkbox' />{' '}
              <label title={available}>{available} </label>
            </div>
          ))}
        </div>
      ),
