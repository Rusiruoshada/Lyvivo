...
import { categorySelectAction } from '../../store/slices/categorySlice.ts';
...
const CategorySpace: React.FC = () => {
  ...
  const handleOnCategoryChange:any = (event: any) => {
    if (event.target.ariaLabel === 'All') {
      ...
      dispatch(
        categorySelectAction.changeCategory({ id: 2, title: 'Grocery' })
      );
    } else if (event.target.ariaLabel === 'Pharmacy') {
      dispatch(
        categorySelectAction.changeCategory({ id: 3, title: 'Pharmacy' })
      );
      ...
      dispatch(
        categorySelectAction.changeCategory({ id: 5, title: 'Electronics' })
      );
    }
  };
      ...
      <Row gutter={20} justify='center' className='p-0 gap-3'>
        {categories1.map((categoryItem) => (
          ...
          >
            <Card.Grid
              style={gridStyle}
              className='p-0 h-fit items-center flex shadow-none'
            >
              <CategoryCard
